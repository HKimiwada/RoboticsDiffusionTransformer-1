...
class RoboticDiffusionTransformerModel(object):
    ...
    def __init__(
        ...
        # If user passes "precomputed" (or None/""), we bypass T5 entirely.
        self._text_disabled = str(pretrained_text_encoder_name_or_path).lower() in ("precomputed", "none", "")
    ...
    def parameters(self, recurse: bool = True):
        """Expose parameters of the underlying torch module so callers
        like eval scripts can do `next(policy.parameters())`."""
        return self.policy.parameters(recurse=recurse)
    ...
    def get_policy(self):
        ...
        _model = RDTRunner(
            ...
            img_pos_embed_config=[
                # No initial pos embed in the last grid size since we've already done in ViT
    ...
    def encode_instruction(self, instruction, device="cuda"):
        ...
        Returns:
            pred: a tensor of latent embeddings of shape (text_max_length, hidden_dim)
        """
        if self._text_disabled or self.text_model is None:
            raise RuntimeError(
                "Text encoder is disabled (precomputed mode). "
                "Provide `text_embeds` directly to `step()`."
            )
    ...
    def _format_joint_to_state(self, joints):
        ...
        Args:
            joints (torch.Tensor): qpos ([B, N, 14]).

        Returns:
            state (torch.Tensor): formatted state for RDT ([B, N, 128]).
        """
        # normalize to [-1, 1]
        ...
        # assemble the unified state vector
    ...
    def step(self, proprio, images, text_embeds):
        ...
        Args:
            ...
            images: RGB images
            text_embeds: instruction embeddings (required in precomputed mode)

        Returns:
            trajectory: predicted joint actions
        """
        device = self.device
        dtype = self.dtype
        ...
        for image in images:
            ...
            if self.image_size is not None:
                image = transforms.Resize(self.image_size)(image)  # fixed from self.data_args.image_size
        ...
        if text_embeds is None:
            if self._text_disabled:
                raise RuntimeError("`text_embeds` is required because the text encoder is disabled (precomputed mode).")
            else:
                raise RuntimeError("`text_embeds` was None. Either provide it, or enable a text encoder and call encode_instruction().")
